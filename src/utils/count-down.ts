...
function setAnimatedTimeout<F extends AnyFunction>(
  callback: F,
  delay: number,
): AnimatedTimeoutID {
  'worklet';
...
function clearAnimatedTimeout(handle: AnimatedTimeoutID): void {
  'worklet';
  removeFromPendingTimeouts(handle);
}

const timeout = {
  setTimeout: setAnimatedTimeout,
  clearTimeout: clearAnimatedTimeout,
};
